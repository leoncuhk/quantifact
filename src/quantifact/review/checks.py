...
def review_frame(task: Task, df: pd.DataFrame) -> list[Finding]:
    ...
    for col in _numeric_columns(task, df):
        ...
        if v.nunique() == 1 and len(v) > 3:
            ...
            severity = (
                "blocking"
                if (len(v) > 20 and task.type != "data_ingestion")
                else "warning"
            )
            out.append(
                Finding(
        ...
        if 0 < null_ratio <= 0.5:
            out.append(Finding(task.name, "note", f"'{col}' is {null_ratio:.0%} null"))
        elif null_ratio > 0.5:
            out.append(
                Finding(task.name, "blocking", f"'{col}' is {null_ratio:.0%} null")
            )
    ...
    if task.index and all(c in df.columns for c in task.index):
        ...
        if dupes:
            out.append(
                Finding(
                    task.name, "blocking", f"{dupes} duplicate rows on index {task.index}"
                )
            )
    ...
    if task.type == "chart":
        ...
        if spec.get("kind") in ("line", "scatter") and len(df) < 2:
            out.append(
                Finding(
                    task.name, "blocking", f"{spec['kind']} chart with {len(df)} point(s)"
                )
            )
