...
UNIVERSE_SPEC: dict[str, tuple[list[str], float, float, float]] = {
    "equity": (["SPX", "NDX", "SX5E", "DAX", "UKX", "NKY", "HSI", "KOSPI",
                "TSX", "ASX", "IBOV", "SENSEX"], -0.55, 0.07, 0.16),
    "bond": (["UST2Y", "UST10Y", "UST30Y", "BUND10Y", "JGB10Y", "GILT10Y",
              "OAT10Y", "BTP10Y", "AUS10Y", "CAN10Y"], -0.18, 0.03, 0.06),
    "fx": (["EURUSD", "USDJPY", "GBPUSD", "AUDUSD", "USDCAD", "USDCHF",
            "USDNOK", "USDMXN", "USDBRL", "USDINR", "USDKRW", "USDZAR"],
           0.12, 0.0, 0.09),
    "commodity": (["BRENT", "WTI", "NATGAS", "GOLD", "SILVER", "COPPER",
                   "ALUM", "CORN", "WHEAT", "SOY", "COTTON"], 0.85, 0.02, 0.24),
    "credit": (["USIG", "USHY", "EURIG", "EURHY", "EMSOV"], -0.25, 0.04, 0.08),
    "inflation": (["US10YTIPS", "US5Y5YBE", "UKILB", "EURILB", "JPBE"],
                  0.35, 0.02, 0.05),
}
...
MACRO_SPEC: dict[str, tuple[str, str, tuple[float, float], float, float, int]] = {
    ...
    "US.CPI.HEADLINE.YOY": ("US headline CPI, year over year", "%", (0.5, 6.0),
                            0.4, 1.8, 14),
    "US.CPI.CORE.YOY": ("US core CPI, year over year", "%", (1.0, 4.5), 0.25, 0.7, 14),
    "US.NET_ENERGY_TRADE": ("US net energy trade balance", "% of GDP", (-2.5, 1.5),
                            0.2, -0.5, 60),
}
...
def universe() -> pd.DataFrame:
    ...
    rows.append({"market_id": _market_id(name, ac), "asset_class": ac,
                 "beta_oil": -0.4, "drift": 0.05, "vol": 0.28,
                 "listed_from": pd.Timestamp("1988-01-01"),
                 "delisted_on": pd.Timestamp(died)})
    name, ac, born = LISTED_LATE
    rows.append({"market_id": _market_id(name, ac), "asset_class": ac,
                 "beta_oil": -0.6, "drift": 0.09, "vol": 0.22,
                 "listed_from": pd.Timestamp(born), "delisted_on": pd.NaT})
...
def _shock_path(dates: pd.DatetimeIndex, beta: float,
...
def build_store(root: str | Path, seed: int = 42) -> SeriesStore:
    ...
    for row in uni.itertuples(index=False):
        window = days[(days >= row.listed_from)
        ...
        store.write(SeriesMeta(
            ...
            description=(f"Daily total return index for {row.market_id} "
                         f"({row.asset_class}), rebased to 100 at listing"),
            frequency="D", unit="index", source="vendor",
            license_tag="demo-synthetic", asset_class=row.asset_class,
            geography="global", owner="market-data", pub_lag_days=0,
    ...
    for sid, (name, unit, (lo, hi), noise, beta, lag) in MACRO_SPEC.items():
        ...
        store.write(SeriesMeta(
            ...
            frequency="M", unit=unit, currency="USD", geography="US",
            source="internal-model", license_tag="demo-synthetic",
            owner="us-macro-team", pub_lag_days=lag,
    ...
    store.write(SeriesMeta(
        series_id="US.CPI.HEADLINE.INDEX", name="US headline CPI index",
        description="US headline consumer price index, level (1982-84 = 100)",
        frequency="M", unit="index", currency="USD", geography="US",
        source="vendor", license_tag="demo-synthetic", owner="market-data",
        pub_lag_days=14, aliases=["cpi", "headline cpi"],
    ...
    store.write(SeriesMeta(
        ...
        frequency="M", unit="%", currency="USD", geography="US",
        source="vendor", license_tag="demo-synthetic", owner="market-data",
        pub_lag_days=14, aliases=["cpi yoy bps"],
    ), pd.Series(np.clip(np.linspace(120, 480, m) + rng.standard_normal(m) * 40,
                         0, 1500), index=months))

    quarters = pd.date_range("1985-03-31", "2026-06-30", freq="QE")
    q = len(quarters)
    store.write(SeriesMeta(
        series_id="US.OUTPUT_GAP.Q", name="US output gap, quarterly vintage",
        description="US output gap, quarterly frequency, vintage series",
        frequency="Q", unit="% of potential", currency="USD", geography="US",
        source="vendor", license_tag="demo-synthetic", owner="market-data",
        pub_lag_days=75, aliases=["output gap quarterly"],
    ), pd.Series(np.linspace(-1.0, 1.5, q) + rng.standard_normal(q) * 0.4,
                 index=quarters))

    # --- entitlement-gated ------------------------------------------------
    store.write(SeriesMeta(
        series_id="BW.POSITIONS.AGGREGATE", name="Aggregate book positioning",
        description="Aggregate positioning across all markets, firm confidential",
        frequency="D", unit="notional", currency="USD", source="internal-model",
        license_tag="internal-confidential", owner="portfolio-construction",
        entitlement_tags=["secure:positions"], pub_lag_days=0,
...
class DemoSyntheticAdapter:
    ...
    def __init__(self, root: str | Path, seed: int = 42):
        root = Path(root)
        self.store = (SeriesStore(root) if (root / "catalog.json").exists()
                      else build_store(root, seed))
        self._universe = universe()[["market_id", "asset_class",
                                     "listed_from", "delisted_on"]]
        self._tables = {
