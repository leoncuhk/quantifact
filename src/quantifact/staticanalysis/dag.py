...
def cross_check(actual: dict[str, list[str]],
                declared: dict[str, list[str]]) -> list[str]:
    problems = []
    for name, deps in actual.items():
        want, got = set(declared.get(name, [])), set(deps)
        if got - want:
            problems.append(f"task '{name}': code consumes undeclared upstream "
                            f"{sorted(got - want)}")
        if want - got:
            problems.append(f"task '{name}': declared dependency {sorted(want - got)} "
