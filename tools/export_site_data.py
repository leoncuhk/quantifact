"""Collect real artefacts from a live run for the interactive run explorer.

Everything the page shows about this project (plan, generated code, cache
...
def main(out: str = "site/data.json") -> int:
    ...
    clarifications = [
        {
            ...
            "why": c.why,
            "options": [
                {"label": o["label"], "recommended": bool(o.get("recommended"))}
                for o in c.options
            ],
        }
    ...
    complaint = (
        "When comparing multiple historical episodes, break the analysis out "
    ...
    earlier = qf.analyse(
        QUESTION, {"as_of": "2026-06-14"}, out=ws / "report-earlier.html", writeback=False
    )
    ...
    data = {
        ...
        "contracts": {
            "passed": sum(v.ok for v in art_taught.verdicts),
            "total": len(art_taught.verdicts),
        },
        "point_in_time": {
            "late": {
                "as_of": art_taught.plan.as_of,
                "episodes": sorted(
                    set(art_taught.result.frames["spine_episodes"]["episode"])
                ),
                "return_cells": len(art_taught.result.frames["market_episode_returns"]),
                "cache_key": art_taught.result.trace("market_prices").cache_key[:10],
            },
            "early": {
                "as_of": earlier.plan.as_of,
                "episodes": sorted(
                    set(earlier.result.frames["spine_episodes"]["episode"])
                ),
                "return_cells": len(earlier.result.frames["market_episode_returns"]),
        ...
        "bench": bench.to_dict(),
        "review": [
            {"task": f.task, "severity": f.severity, "message": f.message}
