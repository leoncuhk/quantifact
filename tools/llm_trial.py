...
def grade(pa, plan, codes, oracle_frames, label, debugger=None, max_fix=1):
    ...
    harness = ExecutionHarness(
        pa.adapter, ValueCache(pa.ws / f"cache-{label}", enabled=False)
    )
                ...
                if not v0.ok:
                    if debugger and attempt < max_fix:
                        codes[name] = debugger.edit(
                            task, codes[name], v0, upstream=schemas_of(plan, task)
                        )
                        row["fix_rounds"] += 1
                        continue
                    break
...
def main() -> int:
    ...
    oracle = ExecutionHarness(
        qf.adapter, ValueCache(qf.ws / "cache-ref", enabled=False)
    ).run(plan, ref_codes)
    ...
    if len(round_codes) > 1:
        ...
        result["determinism"] = {
            ...
            "matched_oracle_both_rounds": sum(
                1
                for k in a
                if next(x for x in result["rounds"][0]["rows"] if x["task"] == k)[
                    "values_match"
                ]
                and next(x for x in result["rounds"][1]["rows"] if x["task"] == k)[
                    "values_match"
                ]
            ),
            "tasks": len(a),
    ...
    print(
        json.dumps(
            {k: v for k, v in result.items() if k in ("determinism", "usage")}, indent=1
        )
    )
    return 0
