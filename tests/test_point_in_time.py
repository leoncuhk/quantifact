...
def test_harness_binds_the_loader_to_the_plan(qf, tmp_path):
    ...
    task = Task(
        ...
        columns=[
            ColumnSpec("date", "obs date", "datetime64[ns]", role="observation_date"),
            ColumnSpec("price", "level", "float64", "index"),
        ],
        index=["date"],
        row_expectation="one row per day",
    )
    src = (
...
def test_dated_columns_may_not_post_date_the_knowledge_date():
    task = toy_task(
        columns=[ColumnSpec("date", "obs", "datetime64[ns]", role="observation_date")],
        index=["date"],
    )
...
def test_episode_set_follows_the_knowledge_date(qf):
    ...
    early_eps = next(a for a in early.resolved_assumptions if a.startswith("Episodes"))
    late_eps = next(a for a in late.resolved_assumptions if a.startswith("Episodes"))
