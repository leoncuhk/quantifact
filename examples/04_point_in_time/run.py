...
def main() -> int:
    ...
    for as_of in ("2022-03-01", "2026-08-01"):
        ...
        print(f"  as_of {as_of}: {len(s):>5} observations, latest "
              f"{s.index.max().date()}")
    print("  Generated code never sees the rows beyond its knowledge date;"
    ...
    task = Task(name="invented", type="table_logic", description="synthesised dates",
                depends_on=["upstream"],
                columns=[ColumnSpec("date", "observation date", "datetime64[ns]",
                                    role="observation_date"),
                         ColumnSpec("value", "a number", "float64")],
                index=["date"], row_expectation="one row per month",
                invariants=[{"kind": "no_future_observations"}])
    df = pd.DataFrame({"date": pd.date_range("2026-01-31", periods=12, freq="ME"),
                       "value": range(12)}).astype({"value": "float64"})
    ...
    for as_of in ("2026-06-14", "2026-08-01"):
        art = qf.analyse(QUESTION, {"as_of": as_of},
                         out=ws / f"report-{as_of}.html")
        episodes = art.result.frames["spine_episodes"]["episode"].tolist()
        rows = len(art.result.frames["market_episode_returns"])
        print(f"  as_of {as_of}: {len(art.plan.tasks)} tasks, "
